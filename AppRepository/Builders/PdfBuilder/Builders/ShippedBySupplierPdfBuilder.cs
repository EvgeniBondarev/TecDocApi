...
namespace Servcies.Builders.PdfBuilder.Builders;

public class ShippedBySupplierPdfBuilder: IPdfBuilder
{
    ...
    public ShippedBySupplierPdfBuilder()
    {
    ...
    public byte[] GetPdf()
    {
        try
        {
           ...
           // Информация об отправителе (остается одна для всех заказов)
            ...
            var supplier = _orders?.FirstOrDefault()?.Supplier;
            var supplierName = supplier?.Name;
            string ndsData = string.Empty;
            if (supplier.IsVatApplicable)
            {
                ndsData = "C налогом (НДС)";
            }
            else
            {
                ndsData = "Без налога (НДС)";
            }
            _document.Add(new Paragraph($"Поставщик: {supplierName}")
                .SetFont(_font)
                .SetFontSize(11));
            ...
            if (ordersByClient != null && ordersByClient.Any())
            {
                foreach (var clientGroup in ordersByClient)
                {
                    ...
                    _document.Add(new Paragraph($"Покупатель: {receiver} ({client.Name})")
                        .SetFont(_font)
                        .SetFontSize(11)
                        .SetMarginTop(15)
                        .SetMarginBottom(5));
                    ...
                    Table table = new Table(UnitValue.CreatePercentArray(new float[] { 5, 12, 35, 10, 14, 14, 10 }))
                    ...
                    // Заголовки таблицы
                    // Заголовки таблицы
                    table.AddHeaderCell(new Cell().Add(new Paragraph("№")).AddStyle(headerStyle));
                    table.AddHeaderCell(new Cell().Add(new Paragraph("Код")).AddStyle(headerStyle));
                    table.AddHeaderCell(new Cell().Add(new Paragraph("Товары")).AddStyle(headerStyle));
                    table.AddHeaderCell(new Cell().Add(new Paragraph("Количество")).AddStyle(headerStyle));
                    table.AddHeaderCell(new Cell().Add(new Paragraph("Остаток")).AddStyle(headerStyle));
                    ...
                    foreach (var order in clientOrders)
                    {
                        ...
                        var sum = order.Quantity * order.PurchasePrice;
                        total += sum;
                        table.AddCell(new Cell().Add(
                            new Paragraph(((decimal)order.PurchasePrice).ToString("N2"))
                                .SetFontSize(8)
                        ...
                        table.AddCell(new Cell().Add(
                            new Paragraph(((decimal)sum).ToString("N2"))
                                .SetFontSize(8)
                                .SetTextAlignment(TextAlignment.RIGHT)
                                .SetFont(_font)
                        ));
            ...
            _document.Add(new Paragraph($"Итого: {total:N2} руб. {ndsData}.")
                .SetFont(_boldFont)
                .SetTextAlignment(TextAlignment.RIGHT));

            _document.Add(new Paragraph($"Всего наименований {_orders.Count}, на сумму {total:N2} руб.")
                .SetFont(_font));
